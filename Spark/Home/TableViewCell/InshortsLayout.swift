    ...
    private func setAttributes() {
        ...
        for item in 0..<self.numberOfItems {
            ...
            if (indexPath.item == self.currentItemIndex + 1) && (indexPath.item < self.numberOfItems) {
                ...
                if SessionManager.sharedInstance.isBoolForInshortsLayout {
                    attribute.center.y = (collectionViewCenterYOffset +  self.collectionViewYOffset) - (screenHeight <= 667 ? 65 : 93)
                    
                } else {
                    attribute.center.y = (collectionViewCenterYOffset +  self.collectionViewYOffset) - (screenHeight <= 667 ? 155 : 185)
                ...
                attribute.frame = CGRect.init(x: 0, y: self.yOffset, width: self.itemWidth, height: self.itemHeight)
                if SessionManager.sharedInstance.isBoolForInshortsLayout {
                    attribute.center.y = (collectionViewCenterYOffset + yOffset) - (screenHeight <= 667 ? 65 : 93)
                    
                }else {
                    attribute.center.y = (collectionViewCenterYOffset + yOffset) - (screenHeight <= 667 ? 155 : 185)
