...
protocol getPlayerVideoAudioIndexDelegate : AnyObject{
...
/**
 This cell is used to set the post in home view page and feed search page which is mentioned as nft and normal post
 */
class NewsCardCollectionCell: UICollectionViewCell, CAAnimationDelegate,ShimmeringViewProtocol, NetworkSpeedProviderDelegate, UIGestureRecognizerDelegate {
...
//
    ...
    var viewModelCell:NewsCardViewModel? {
        
        didSet {
            print("It is From Cell\(viewModelCell?.monetize ?? 0)")
           // print()
        }
    ...
    ///Show the activity indicator view with loader
    ...
    /// On Clicking left button it will helps to repost for the current post
    @IBAction func leftViewButtonTapped(_ sender: UIButton) {
        self.leftView.isHidden = true
        collectionDelegate?.leftButtonDetailsTapped(sender: sender)
    }
    ...
    /// On pressing the profile label button open the other user profile page
...
//        dummyView11.layer.cornerRadius = 6
    ...
    /// Load animated image of gif for clapping
    ...
    ///Check is the type true for check the media type
...
/**
 Set all the button clickable here
 */
extension NewsCardCollectionCell {
...
extension NewsCardCollectionCell: AVPlayerViewControllerDelegate {
