...
open class MIService: UNNotificationServiceExtension {
    ...
    open override func didReceive(_ request: UNNotificationRequest, withContentHandler contentHandler: @escaping (UNNotificationContent) -> Void)
    {
        ...
        if let bestAttemptContent = bestAttemptContent {
         ...
         //   guard let body = bestAttemptContent.userInfo["gcm.notification.fcm_options"] as? Dictionary<String, Any>, let imageUrl = body["image"] as? String else { fatalError("Image Link not found") }
           //imageUrl = "http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/TearsOfSteel.mp4"
            
            print(bestAttemptContent)
            
            guard let imageUrl = bestAttemptContent.userInfo["gcm.notification.imageUrl"] as? String else { fatalError("Image Link not found") }
